...
app.post('/', upload.single('pdfFile'), async (req, res) => {
    ...
    extract(filePath, async function (err, text) {
        if (err) {
            console.dir(err)
            return
        }
        ...
        while ((mcqMatches = mcqRegex.exec(text.join(''))) !== null) {
            mcqResults.push({
                QuestionType: 'MCQ',
        ...
        while ((saMatches = saRegex.exec(text.join(''))) !== null) {
            saResults.push({
                QuestionType: 'SA',
        ...
        res.json({ result, right, wrong, answered })
    })
})

app.listen(3001, (err) => err ? console.log(err) : console.log("server started : 3001"))